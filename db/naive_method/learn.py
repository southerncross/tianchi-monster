...
test_feature_i = {}
max_recall = 0
fp = open('train/naive_method_%d_3_2.csv'%test, 'r')
for line in fp:
...
for i in day:
    print 'train/naive_method_%d_3_2.csv'%i
    if i >= test:
        break
    tag = {}
    ...
    fp = open('train/naive_method_%d_3_2.csv'%i, 'r')
    for line in fp:
        (ui, ii) = line.strip().split(',')
        cur = []
        cur += user_rate[ui]
...
# reswriter = csv.writer(open("naive_method_2_2.csv", "wb"))
result = 0
find = 0
