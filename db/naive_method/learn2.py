...
test_feature_i = {}
max_recall = 0
fp = open('train2/naive_method_%d_2.csv'%test, 'r')
for line in fp:
...
for i in day:
    print 'train2/naive_method_%d_2.csv'%i
    if i >= test:
        break
    tag = {}
    ...
    fp = open('train2/naive_method_%d_2.csv'%i, 'r')
    for line in fp:
        (ui, ii) = line.strip().split(',')
        cur = []
        cur += user_rate[ui]
...
# reswriter = csv.writer(open("naive_method_2_new.csv", "wb"))
result = 0
find = 0
